...
def generate_presentation_content(topic, extracted_text, num_slides, style):
    client = genai.Client(api_key=os.getenv("GOOGLE_API_KEY"))

    try:
        prompt = f"""
        You are an expert presentation creator. Given the extracted text, create a detailed PowerPoint presentation outline based strictly on its content. Do not add any external information.
        The extracted text is as follows: {extracted_text}
        The topic of the presentation should be: "{topic}".
...
st.title("📑 AI Presentation Generator from Files")
...
if submitted:
    try:
        ...
        extracted_text = extract_text_from_files([uploaded_file])
        content = generate_presentation_content(topic, extracted_text, num_slides, style)
